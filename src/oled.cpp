...
bool get_xy(int16_t x, int16_t y) {
    // TODO
    uint16_t ndx = x + (y>>3)*WIDTH; // index of the byte to access
    uint16_t color = buffer[ndx] &  (1 << (y&7));
    color = (color >> y&7);
    bool out = color;
    return out;
}
...
void SSD1306::draw_rect(uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t color) {
    ...
    drawFastVLineInternal(x+width, y,height,color);

    // draw horizontal lines
    drawFastHLineInternal(x, y,width,color);
    drawFastHLineInternal(x, y+height,width,color);

}

void SSD1306::fill_rect(uint8_t x, uint8_t y, uint8_t width, uint8_t height, uint8_t color) {
    // using vertical lines, since they are faster
    for (uint8_t a = 0; a < (width+1); a+=1)
        {
        drawFastVLineInternal(x+a, y,height,color);
        }
}
...
void SSD1306::draw_line(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t color) {
    ...
    if (deltax==0){
        if (deltay<0){
            drawFastVLineInternal(x2, y2,-deltay,color);
        }
        else{
            drawFastVLineInternal(x1, y1,deltay,color);
        }

    }
    else{
        if(deltay==0){
            drawFastHLineInternal(x1, y1,x2-x1,color);
        }
        else{
            float deltaerror = deltay/deltax;
            deltaerror = (deltaerror>0)?deltaerror:-deltaerror;
